"""Standalone entrypoint for the Telegram bot (copy of tools/telegram_bot.py).

This file is intended to be built into its own Docker image under
`backend/telegram_bot` so the bot can run independently with a dedicated
docker-compose file.
"""
import os
import logging
...
def build_system_prompt() -> List[Dict[str, str]]:
    """Build the system prompt describing the project and desired assistant behavior."""
    project_context = (
...
async def call_chatgpt(user_text: str) -> str:
    """Call OpenAI ChatCompletion API in a thread to avoid blocking the event loop."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # show quick menu buttons on start
    keyboard = [
...
async def saludar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Predefined greeting command for demo purposes."""
    await update.message.reply_text("hoolisss estrellitas")


async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send an inline keyboard with predefined questions."""
    keyboard = [
        [InlineKeyboardButton("Saludar", callback_data="saludar")],
        [
            InlineKeyboardButton("Objetivo", callback_data="objetivo"),
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # For other keys, use the ChatGPT API (with prompts from PROMPT_MAP)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Reply including the menu so users always see predefined questions
